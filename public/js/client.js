 ...
 * @author  Marina Fokina - skyangel0421@gmail.com
 * @version 1.0.1
 *
 */
...
const welcomeImg = '../images/logo.png';
const shareUrlImg = '../images/logo.png';
const leaveRoomImg = '../images/leave-room.png';
const confirmImg = '../images/logo.png';
...
const buttons = {
    main: {
        ...
        showLockmyRoomBtn: true,
        showUnLockmyRoomBtn: true
    },
    chat: {
...
let recStartTime;
let recElapsedTime;
let mirotalkTheme = 'white'; // dark - grey ...
...
function getHtmlElementsById() {
    ...
    lockmyRoomBtn = getId('lockmyRoom');
    unlockmyRoomBtn = getId('unlockmyRoom');
    // chat Room elements
...
function setButtonsToolTip() {
    ...
    setTippy(lockmyRoomBtn, 'lock this room', 'right-start');
    setTippy(unlockmyRoomBtn, 'unlock this room', 'right-start');
    // chat room buttons
...
function handleRules(isPresenter) {
    ...
    if (!isPresenter) {
        ...
        buttons.main.showLockmyRoomBtn = false;
        buttons.main.showUnLockmyRoomBtn = false;
        //...
    } else {
        buttons.settings.showTabRoomParticipants = true;
        buttons.settings.showTabRoomSecurity = true;
        buttons.main.showLockmyRoomBtn = true;
        buttons.main.showUnLockmyRoomBtn = isRoomLocked;
...
function handleButtonsRule() {
    ...
    elemDisplay(lockmyRoomBtn, buttons.main.showLockmyRoomBtn);
    elemDisplay(unlockmyRoomBtn, buttons.main.showUnLockmyRoomBtn);
    // elemDisplay(showLockmyRoomBtn, buttons.main.showLockmyRoomBtn)
    // chat
...
function welcomeUser() {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let message = {
                email: '',
                subject: 'Please join our ONEOD Video Chat Meeting',
...
function setTheme(theme) {
    ...
    switch (mirotalkTheme) {
        ...
        case 'white':
            // dark theme
            swalBackground = '#111';
            document.documentElement.style.setProperty('--body-bg', '#ddd');
            document.documentElement.style.setProperty('--msger-bg', '#444');
            document.documentElement.style.setProperty('--msger-private-bg', '#444');
            document.documentElement.style.setProperty('--wb-bg', 'radial-gradient(#FFFFFF, #FFFFFF)');
            document.documentElement.style.setProperty('--box-shadow', '0px 8px 16px 0px rgba(0, 0, 0, 0.2)');
            document.documentElement.style.setProperty('--left-msg-bg', '#252d31');
            document.documentElement.style.setProperty('--right-msg-bg', '#056162');
            document.documentElement.style.setProperty('--private-msg-bg', '#6b1226');
...
async function initEnumerateDevices() {
    ...
    if (!useAudio && !useVideo) {
        userLog('toast', 'Oops, Something went Wrong!!');
        openURL(
            `/`,
        );
    }
...
async function initEnumerateAudioDevices() {
    console.log('005.', isEnumerateAudioDevices)
...
async function setupLocalMedia() {
    ...
    // let videoConstraints = useVideo ? getVideoConstraints('default') : true;
...
function setupMySettings() {
    ...
    lockmyRoomBtn.addEventListener('click', (e) => {
        handleRoomAction({ action: 'lock' }, true);
    });
    unlockmyRoomBtn.addEventListener('click', (e) => {
        handleRoomAction({ action: 'unlock' }, true);
    });
}
...
function showCaptionDraggable() {
    ...
    captionBtn.className = 'fas fa-list-alt';
...
function showAbout() {
    ...
    Swal.fire({
        background: swalBackground,
        position: 'center',
        title: '<strong>ONEDO P2P</strong>',
        imageAlt: 'ONEDO Image',
        imageUrl: aboutImg,
        html: `
        <br/>
        <div id="about">
            <br/><br/>
            <button class="pulsate umami--click--sponsor" onclick="window.open('/about')"><i class="fas fa-heart" ></i>&nbsp;About</button>
            <br /><br />
            Author:<a href="#" class="umami--click--linkedin" target="_blank"> Marina Fikina </a>
        </div>
        `,
        showClass: {
            popup: 'animate__animated animate__fadeInDown',
...
function leaveFeedback() {
    Swal.fire({
        ...
        title: 'Are you sure?',
        text: 'Do you want to leave this room now?',
        confirmButtonText: `Yes`,
        denyButtonText: `No`,
        showClass: {
            popup: 'animate__animated animate__fadeInDown',
